  ...
  <div class="shadow-md bg-white py-4 px-4 mx-auto">
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
      <table class="w-full text-sm text-left rtl:text-right text-gray-500">
        ...
        <tbody>
          <tr v-for="loan in loans" :key="loan.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
            ...
            <td class="px-6 py-4 flex items-center gap-2">
              ...
              <button v-if="loan.status === 'pending'" @click="$emit('reject', loan.id)"
                class="font-medium text-red-600 hover:underline">
                Reject
              </button>
              <button v-if="loan.status === 'approved'" @click="$emit('repaid', loan.id)"
